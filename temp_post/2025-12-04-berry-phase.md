...
각 점에 $\psi_n(R)$을 대응시키는 대신, $\psi_n(R)$이 생성한 1차원 복소 벡터 공간 $\text{span}\\{\psi_n(R)\\}$을 대응시키는 것이다. 이를 **복소 선다발(complex line bundle)**이라 부른다.
게이지를 선택하는 것은 이 선다발의 단면(section)을 고르는 것과 같다. (즉, 각 점에서 하나의 벡터를 고르는 것)
