...
> 
> 1. $\mu(\varnothing) = 0$
> 2. 서로소인 가측 집합들의 열 $\left\{A_i\right\}_{i=1}^{\infty} \subseteq \mathcal{F}$에 대해,
> 
> $$
...
이 공에 각각 번호를 붙여 $\Omega = \left\{1, 2, \ldots, 100 \right\}$가 되도록 하자.
...
다만, 모든 사건을 모아둔 집합 $\mathcal{F}$와는 달리, 확률 변수 $X$가 만들어내는 사건들은 그 일부에 불과하다. 가령, $X^{-1}(B)$는 $B$에 1이 포함되어있다면 반드시 1부터 20까지의 수를 모두 포함하여야 하므로, $\left\{1, 21\right\}$이라는 사건은 $X$의 역상으로 표현할 수 없다.
